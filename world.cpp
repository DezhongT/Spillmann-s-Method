...
world::world(setInput &m_inputData)
{
	...
	// GJ = m_inputData.GetScalarOpt("GJ");
    ...
    // youngM = 4.0 * EI /(M_PI * pow(rodRadius, 4));
    // shearM = 2.0 * GJ/ (M_PI * pow(rodRadius, 4));

    double EI_b = 1.8e4 * M_PI * pow(1.6e-3, 4)/4;

    // density = EI/EI_b * 100;
    // viscosity = EI/EI_b * 0.01;
	// cout<<"EI: "<<EI<<endl;
	// cout<<"GJ: "<<GJ<<endl;
    // cout<<"youngM: "<<youngM<<endl;
    // cout<<"shearM: "<<shearM<<endl;
    // cout<<"density: "<<density<<endl;
    // cout<<"viscosity: "<<viscosity<<endl;
    // cout<<"friction: "<<friction<<endl;
    // exit(0);
 //    cout<<"ratio: "<<EI/GJ<<endl;
...
void world::CoutDataC(ofstream &outfile)
{
    ...
    double torsion;
    torsion = check->torsion;

	outfile<<currentTime<<" "<< f << " " <<f1<<" "<< e1<<" "<< loop <<" "<<braid<<" "<<velocity<<" " <<torsion<<endl;	
}
...
void world::updateBoundary()
{
    ...
    // cout<<"v: "<<v_constant<<endl;
    // cout<<"statictime: "<<statictime<<endl;
    // cout<<"pulltime: "<<pulltime<<endl;
    
    
	if (currentTime>statictime && currentTime<= statictime+pulltime)
	{
		// rod->setThetaBoundaryCondition(rod->getTheta(0) + omega * deltaTime, 0);
		// rod->setThetaBoundaryCondition(rod->getTheta(numVertices-2) + omega * deltaTime,numVertices-2);




		rod->setVertexBoundaryCondition(rod->getVertex(0)-u*deltaTime + u1 *deltaTime,0);
		rod->setVertexBoundaryCondition(rod->getVertex(1)-u*deltaTime + u1 * deltaTime,1);

		rod->setVertexBoundaryCondition(rod->getVertex(numVertices-2)+u*deltaTime + u1*deltaTime ,numVertices-2);
		rod->setVertexBoundaryCondition(rod->getVertex(numVertices-1)+u*deltaTime + u1 *deltaTime,numVertices-1);
