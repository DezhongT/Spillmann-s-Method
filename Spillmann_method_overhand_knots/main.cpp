...
void display(void)
{
	...
	while ( myWorld.simulationRunning() > 0)
	{
        ...
        if (int(myWorld.getCurrentTime()/1e-3)%100==0)
		{   
			//record the configuration of rod per 0.1 s
			myWorld.CoutData(outfile1); 
	    }
	    //record the force and shortening
...
int main(int argc,char *argv[])
{
	...
	else
	{
		while ( myWorld.simulationRunning() > 0)
		{
			...
			if (int(myWorld.getCurrentTime()/1e-3)%100==0)
			{
			   myWorld.CoutData(outfile1); // write data to file
			}
			myWorld.CoutDataC(outfile); 
		}
	}
